...
func (w *walker) applyNormalization(path, normPath string, info fs.FileInfo) (string, error) {
	...
	if w.Filesystem.SameFile(info, normInfo) {
		...
		// We don't want to use the standard sync prefix here, as that will result in the file being ignored
		// and eventually deleted by Vidsync if the rename back fails.
