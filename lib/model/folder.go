...
import (
	...
	"github.com/CrownLedger/vidsync/lib/stringutil"
	"github.com/CrownLedger/vidsync/lib/svcutil"
	"github.com/CrownLedger/vidsync/lib/versioner"
	"github.com/CrownLedger/vidsync/lib/watchaggregator"
)

// Arbitrary limit that triggers a warning on kqueue systems
const kqueueItemCountThreshold = 10000

// healthCheckInterval governs how often the folder serve loop checks
// the project root / marker dir for existence so a missing path or
// missing marker surfaces in the folder error state within seconds
// instead of waiting for the (hourly) full rescan. The check is a
// cheap two stat() calls.
...
func newFolder(model *model, ignores *ignore.Matcher, cfg config.FolderConfiguration, evLogger events.Logger, ioLimiter *semaphore.Semaphore, ver versioner.Versioner) *folder {
	f := folder{
		...
		// healthCheckTimer ticks every 30s so the folder error state
		// reacts to the user removing/renaming the project root or
		// the .vidsync marker without waiting for the (hourly) scan.
		// Those paths are in the daemon's internals list and don't
		// surface through the fs watcher; a periodic CheckPath is
...
func (f *folder) Serve(ctx context.Context) error {
	...
	for {
		...
		case <-f.healthCheckTimer.C:
			// Lightweight tick: re-check path + marker so a deleted
			// project root or .vidsync directory propagates to the
			// folder error state without waiting for the next scan.
			// The folder error state is what drives the UI Resync
			// button next to the Activity tab.
			err = f.healthTimerFired(ctx)
		}
...
// healthTimerFired runs CheckPath + the minimum-free-space check and
// updates the folder error state accordingly. Always re-arms the
// timer. Returned errors flow to the serve loop's setError block so
// the folder state reflects the latest health state on every tick.
func (f *folder) healthTimerFired(ctx context.Context) error {
	defer f.healthCheckTimer.Reset(healthCheckInterval)
	if err := f.getHealthErrorWithoutIgnores(); err != nil {
		return err
	}
	// All good — make sure any stale error from a previous tick or
	// scan is cleared so the UI dismisses the Resync button as soon
	// as the user has restored the path / marker.
	f.setError(ctx, nil)
	return nil
}
