...
func (c *folderSummaryService) processUpdate(ev events.Event) {
	...
	case events.StateChanged:
		...
		// Surface transitions in and out of the "error" state right
		// away so the UI's error pill and Resync button track the
		// folder's actual health (otherwise the FolderSummary lags
		// behind /rest/db/status by however long until the next
		// triggered emit, and the UI looks stuck).
		...
		if from != "syncing" && from != "sync-preparing" {
			return
		}
