...
import (
	...
	"github.com/CrownLedger/vidsync/lib/ignore"
	"github.com/CrownLedger/vidsync/lib/osutil"
	...
	"github.com/CrownLedger/vidsync/lib/semaphore"
	"github.com/CrownLedger/vidsync/lib/stats"
	"github.com/CrownLedger/vidsync/lib/svcutil"
	"github.com/CrownLedger/vidsync/lib/ur/contract"
	"github.com/CrownLedger/vidsync/lib/versioner"
)
...
type Model interface {
	...
	LocalDirectoryTree(folder, prefix string, levels int, dirsOnly bool) ([]*TreeEntry, error)
...
func (m *model) removeFolder(cfg config.FolderConfiguration) {
	...
	if isPathUnique {
		...
		// otherwise not removable) Vidsync-specific marker files.
...
func (m *model) GlobalDirectoryTree(folder, prefix string, levels int, dirsOnly bool) ([]*TreeEntry, error) {
	return m.directoryTree(folder, prefix, levels, dirsOnly, false)
}

// LocalDirectoryTree mirrors GlobalDirectoryTree but iterates the
// local device's file index, so the result reflects what's actually
// on disk for this peer. Same shape, same dirsOnly / levels / prefix
// semantics. Used by /rest/db/browse?local=true to give the UI a view
// that drops files we've deleted locally but haven't yet propagated
// to the global state (e.g. send-and-receive when the peer hasn't
// acked the deletion yet).
func (m *model) LocalDirectoryTree(folder, prefix string, levels int, dirsOnly bool) ([]*TreeEntry, error) {
	return m.directoryTree(folder, prefix, levels, dirsOnly, true)
}

func (m *model) directoryTree(folder, prefix string, levels int, dirsOnly, localOnly bool) ([]*TreeEntry, error) {
	m.mut.RLock()
	_, ok := m.folderCfgs[folder]
	m.mut.RUnlock()
	if !ok {
	...
	if !localOnly {
		for f, err := range itererr.Zip(m.sdb.AllGlobalFilesPrefix(folder, prefix)) {
			...
			name := strings.Replace(f.Name, prefix, "", 1)
			if levels > -1 && strings.Count(name, sep) > levels {
				continue
			}
			parent, base, ok, perr := walkParent(root, name, sep)
			if perr != nil {
				return nil, perr
			}
			if !ok {
				return nil, fmt.Errorf("could not find parent for path '%s'", name)
			}
			isDir := f.Type == protocol.FileInfoTypeDirectory
			if dirsOnly && !isDir {
				continue
			}
	...
	// AllLocalFiles has no prefix variant, so iterate everything for
	// this device and filter in-code. Cheap for typical project sizes;
	// can push a prefix-aware iterator into the db layer if it becomes
	// hot. Directories aren't guaranteed to iterate before their
	// children here, so walkParent synthesizes any missing intermediate
	// directory entries on the fly.
	seq, errFn := m.sdb.AllLocalFiles(folder, protocol.LocalDeviceID)
	for f := range seq {
		if f.IsInvalid() || f.Deleted {
			continue
		}
		if prefix != "" {
			...
			if strings.HasPrefix(prefix, f.Name) {
				continue
			}
		}
		name := strings.Replace(f.Name, prefix, "", 1)
		...
		parent, base, _, perr := walkParent(root, name, sep)
		if perr != nil {
			return nil, perr
		}
...
// walkParent descends from root along the directory path of `name`,
// returning the parent node, the leaf basename, and whether the path
// resolved cleanly. In local-only mode we may not have iterated the
// parent directory yet, so missing intermediate dirs are created on
// demand and `ok` stays true. In global mode the caller treats a
// missing parent as a malformed iteration order (returns error).
func walkParent(root *TreeEntry, name, sep string) (*TreeEntry, string, bool, error) {
