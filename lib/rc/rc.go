...
// Package rc provides remote control of a Vidsync process via the REST API.
package rc
...
import (
	...
	"github.com/CrownLedger/vidsync/lib/dialer"
...
// NewProcess returns a new Process talking to Vidsync at the specified address.
...
// Start runs the specified Vidsync binary with the given arguments.
// Vidsync should be configured to provide an API on the address given to
...
// AwaitStartup waits for the Vidsync process to start and perform initial
...
// Stop stops the running Vidsync process. If the process was logging to a
...
// Stopped returns a channel that will be closed when Vidsync has stopped.
