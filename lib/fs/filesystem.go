...
import (
	...
	"github.com/CrownLedger/vidsync/lib/ignore/ignoreresult"
...
// fs cannot import config or versioner, so we hard code the marker
// names (config.DefaultMarkerName + config.LegacyMarkerName) and
// .stversions (versioner.DefaultPath). Both ".vidsync" (current) and
// ".stfolder" (legacy from upstream Syncthing) must stay in this list
// — folders that pre-date the rename still have the old marker on
// disk and we never want to sync that out.
var internals = []string{".vidsync", ".stfolder", ".stignore", ".stversions"}
