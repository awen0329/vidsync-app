...
const (
	WindowsTempPrefix = "~vidsync~"
	UnixTempPrefix    = ".vidsync."

	// Legacy temp prefixes from before the Vidsync rebrand. We never
	// create these anymore, but IsTemporary still recognizes them so any
	// leftover temp files aren't mistaken for real files and synced.
	legacyWindowsTempPrefix = "~syncthing~"
	legacyUnixTempPrefix    = ".syncthing."
)
...
func IsTemporary(name string) bool {
	...
	return strings.HasPrefix(name, WindowsTempPrefix) ||
		...
		strings.HasPrefix(name, legacyWindowsTempPrefix) ||
		strings.HasPrefix(name, legacyUnixTempPrefix)
}
