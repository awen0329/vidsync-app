...
// Package config implements reading and writing of the sync configuration file.
package config
...
import (
	...
	"github.com/CrownLedger/vidsync/lib/netutil"
	"github.com/CrownLedger/vidsync/lib/protocol"
	"github.com/CrownLedger/vidsync/lib/sliceutil"
	"github.com/CrownLedger/vidsync/lib/structutil"
)
...
// CheckFolderPathOverlap returns an error if the candidate folder's root
// path is the same as, nested inside, or contains the root path of any
// *other* configured folder. Two folders sharing a directory tree corrupt
// each other — they fight over the same .stfolder marker and, when their
// sync types differ, deletes/overwrites applied by one are picked up and
// re-propagated by the other — so we refuse to add or move a folder onto
// another's path. The candidate's own ID is excluded, so editing a folder's
// other settings (or re-saving it at the same path) stays allowed.
func (cfg *Configuration) CheckFolderPathOverlap(candidate FolderConfiguration) error {
	cand := canonicalFolderPath(candidate.Path)
	if cand == "" {
		return nil
	}
	...
	for i := range cfg.Folders {
		...
		op := canonicalFolderPath(other.Path)
		if op == "" {
			continue
		}
		switch {
		case cand == op:
			return fmt.Errorf("local folder %q is already used by project %s", candidate.Path, other.Description())
		case strings.HasPrefix(cand, op+sep):
			return fmt.Errorf("local folder %q is inside project %s, which already syncs the parent folder", candidate.Path, other.Description())
		case strings.HasPrefix(op, cand+sep):
			return fmt.Errorf("local folder %q contains project %s, which already syncs a subfolder", candidate.Path, other.Description())
		}
	}
	return nil
}

// canonicalFolderPath normalizes a folder root path for overlap comparison:
// tilde expansion, absolute + cleaned, symlink/firmlink resolution, and
// case-folding on platforms whose filesystems are case-insensitive by
// default (Windows, macOS). Resolving symlinks is what makes aliases of the
// same directory compare equal — e.g. on macOS the boot volume is mounted at
// both / and /Volumes/Macintosh_HD, so /Volumes/Macintosh_HD/Users/x and
// /Users/x are one directory. Returns "" for an empty input so callers can
// skip the check.
func canonicalFolderPath(p string) string {
	if p == "" {
		return ""
	}
	if expanded, err := fs.ExpandTilde(p); err == nil {
		p = expanded
	}
...
// resolveSymlinks resolves symlinks and volume firmlinks in p. EvalSymlinks
// needs the path to exist, so when p is a folder that's about to be created
// we resolve the deepest existing ancestor and re-append the missing tail —
// that way a new project nested under an aliased parent still canonicalizes
// the same as the existing folder it overlaps. Falls back to p unchanged if
