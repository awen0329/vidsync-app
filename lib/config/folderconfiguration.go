...
const (
	DefaultMarkerName          = ".vidsync"
	// LegacyMarkerName is the marker name used by upstream Syncthing
	// before the Vidsync rebrand. We continue to accept it in
	// checkFilesystemPath so folders created by older builds (or by
	// users who imported a project from another Syncthing-family tool)
	// don't suddenly report "marker missing".
	LegacyMarkerName           = ".stfolder"
...
type FolderConfiguration struct {
	...
	FSWatcherDelayS         float64                     `json:"fsWatcherDelayS" xml:"fsWatcherDelayS,attr" default:"1"`
...
func (f *FolderConfiguration) RemoveMarker() error {
	...
	// Legacy: marker dirs created before the rename hold a
	// syncthing-folder-<id>.txt file. Try to remove that too so the
	// outer Remove(DefaultMarkerName) doesn't trip over a non-empty dir.
	h := sha256.Sum256([]byte(f.ID))
	legacy := fmt.Sprintf("syncthing-folder-%x.txt", h[:3])
	_ = ffs.Remove(filepath.Join(DefaultMarkerName, legacy))
	return ffs.Remove(DefaultMarkerName)
}

func (f *FolderConfiguration) markerFilename() string {
	h := sha256.Sum256([]byte(f.ID))
	return fmt.Sprintf("vidsync-folder-%x.txt", h[:3])
}

func (f *FolderConfiguration) markerContents() []byte {
	var buf bytes.Buffer
	buf.WriteString("# This directory is a Vidsync folder marker.\n# Do not delete.\n\n")
...
func (f *FolderConfiguration) checkFilesystemPath(ffs fs.Filesystem, path string) error {
	...
	if err != nil {
		...
		// Legacy fallback: accept the old upstream marker (".stfolder")
		// when the configured marker is the new default. Lets folders
		// created before the Vidsync rename keep working without any
		// on-disk migration. Folders that explicitly opted into a
		// non-default marker (MarkerName != DefaultMarkerName) are not
		// covered — they're already off the default path.
		if f.MarkerName == DefaultMarkerName {
			if _, err2 := ffs.Stat(filepath.Join(path, LegacyMarkerName)); err2 == nil {
				return nil
			}
		}
