...
func migrateToConfigV14(cfg *Configuration) {
	...
	// Vidsync fork: clear the legacy Syncthing releases URL so older
	// configs don't keep pinging syncthing.net infrastructure. Empty
	// disables the auto-upgrade fetcher in this build.
	if cfg.Options.ReleasesURL == "https://api.github.com/repos/syncthing/syncthing/releases?per_page=30" {
		cfg.Options.ReleasesURL = ""
	}
}
