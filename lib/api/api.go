...
import (
	...
	"github.com/CrownLedger/vidsync/internal/slogutil"
	"github.com/CrownLedger/vidsync/lib/build"
	"github.com/CrownLedger/vidsync/lib/config"
	"github.com/CrownLedger/vidsync/lib/connections"
	"github.com/CrownLedger/vidsync/lib/discover"
	...
	"github.com/CrownLedger/vidsync/lib/model"
	...
	"github.com/CrownLedger/vidsync/lib/tlsutil"
	"github.com/CrownLedger/vidsync/lib/upgrade"
	"github.com/CrownLedger/vidsync/lib/ur"
)
...
// InjectedListener, when non-nil, is used by the API service in place
// of opening a network socket. The Vidsync desktop app sets this to an
// in-memory listener so the REST API is reachable only from code inside
// the host process — there is no TCP/unix socket for a browser or any
// other process to connect to. It's a package-level hook rather than a
// New() parameter to avoid changing the constructor signature shared
// with upstream cmd/syncthing. The listener must tolerate being
// Close()d (the service closes its listener on every soft-restart) and
// keep accepting afterwards.
var InjectedListener net.Listener

func (s *service) getListener(guiCfg config.GUIConfiguration) (net.Listener, error) {
	if InjectedListener != nil {
		// In-memory transport: no socket, no TLS — plain HTTP over an
		// in-process pipe. Safe because nothing off-process can reach it.
		return InjectedListener, nil
	}
...
func (s *service) Serve(ctx context.Context) error {
	...
	restMux.HandlerFunc(http.MethodGet, "/rest/folder/file", s.getFolderFile)                 // folder path
	restMux.HandlerFunc(http.MethodGet, "/rest/folder/thumb", s.getFolderThumb)               // folder path — ffmpeg poster/waveform
	restMux.HandlerFunc(http.MethodGet, "/rest/folder/preview", s.getFolderPreview)           // folder path — transcoded H.264 preview clip
	restMux.HandlerFunc(http.MethodHead, "/rest/folder/preview", s.getFolderPreview)          // HEAD: poll proxy readiness (200 ready / 202 generating / 204 none)
	...
	restMux.HandlerFunc(http.MethodPost, "/rest/folder/resync", s.postFolderResync)              // folder
	restMux.HandlerFunc(http.MethodGet, "/rest/system/userdirs", s.getSystemUserDirs)            // -
	restMux.HandlerFunc(http.MethodGet, "/rest/system/diskfree", s.getSystemDiskFree)            // path
...
func corsMiddleware(next http.Handler, allowFrameLoading bool) http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if !allowFrameLoading {
			...
			// Vidsync GUI as part of their own website
...
// getFolderFile streams a single file out of a managed folder. Added
// for Vidsync's media-preview / thumbnail features — browsers can't
// touch local disk, so the daemon serves the bytes over its existing
// authenticated REST surface.
//
// Path safety: the requested path is joined against the folder's
// configured root and the result is checked to make sure it stays
// inside the root. This blocks `..` traversal and absolute paths.
// We deliberately don't dereference symlinks — if a folder contains
// one, http.ServeFile follows it but its target must also be inside
// the root or ServeFile will refuse to serve (its own check).
//
// Range support: http.ServeContent (which ServeFile delegates to)
// already handles HTTP Range requests, so video <video> elements can
// seek and stream without us hand-rolling 206 responses.
// resolveFolderFile maps a (folderID, POSIX relPath) pair to an absolute
// on-disk path inside the folder root, enforcing containment so `..` /
// absolute paths can't escape. Returns the cleaned path, its FileInfo,
// and on failure an HTTP status + error suitable for the response.
// Shared by the file-streaming and thumbnail endpoints.
func (s *service) resolveFolderFile(folderID, relPath string) (string, os.FileInfo, int, error) {
	if folderID == "" || relPath == "" {
		return "", nil, http.StatusBadRequest, errors.New("folder and path query params are required")
	}
	fcfg, ok := s.cfg.Folder(folderID)
	if !ok {
		return "", nil, http.StatusNotFound, errors.New("folder not found")
	}
	// /rest/db/browse hands the frontend POSIX-style paths (slash-
	// separated) even on Windows. Normalize to the host's separator
	// before joining with the folder root.
	root, err := filepath.Abs(fcfg.Path)
	if err != nil {
		return "", nil, http.StatusInternalServerError, fmt.Errorf("bad folder root: %w", err)
	}
	cleaned, err := filepath.Abs(filepath.Join(root, filepath.FromSlash(relPath)))
	if err != nil {
		return "", nil, http.StatusBadRequest, fmt.Errorf("bad path: %w", err)
	}
	// Containment check: cleaned must be the root itself or a path
	// strictly under it. Comparing with a trailing separator on root
	// prevents `/foo/barEvil` from matching root `/foo/bar`.
	if cleaned != root && !strings.HasPrefix(cleaned, root+string(filepath.Separator)) {
		return "", nil, http.StatusForbidden, errors.New("path escapes folder root")
	}
	...
	if info.IsDir() {
		return "", nil, http.StatusBadRequest, errors.New("path is a directory")
	}
	return cleaned, info, 0, nil
}

func (s *service) getFolderFile(w http.ResponseWriter, r *http.Request) {
	qs := r.URL.Query()
	cleaned, _, status, err := s.resolveFolderFile(qs.Get("folder"), qs.Get("path"))
	if err != nil {
		http.Error(w, err.Error(), status)
		return
	}
	// Hint to the browser that this is a transient resource; clients
	// can still cache via ETag (ServeContent emits a Last-Modified).
...
func (s *service) getDBBrowse(w http.ResponseWriter, r *http.Request) {
	...
	// local=true: build the tree from this peer's local index rather
	// than the merged global view, so files this peer has deleted
	// (even ones not yet propagated to other peers) don't appear.
	localOnly := qs.Get("local") != ""

	levels, err := strconv.Atoi(qs.Get("levels"))
	if err != nil {
		levels = -1
	}
	var result []*model.TreeEntry
	if localOnly {
		result, err = s.model.LocalDirectoryTree(folder, prefix, levels, dirsOnly)
	} else {
...
// getSystemUserDirs returns common per-user filesystem locations
// (home, downloads, documents) as absolute native paths. Used by the
// Accept-folder dialog to seed a sensible default destination —
// dropping invited projects into ~/Downloads is the convention users
// expect on every platform.
func (s *service) getSystemUserDirs(w http.ResponseWriter, _ *http.Request) {
	...
	if home != "" {
		out["home"] = home
		out["downloads"] = filepath.Join(home, "Downloads")
		out["documents"] = filepath.Join(home, "Documents")
	}
	sendJSON(w, out)
}

// getSystemDiskFree returns free and total bytes for the disk that
// contains the given path. Walks up to the nearest existing
// directory when the path doesn't exist yet (the user is still
// typing or picked a not-yet-created subfolder).
func (s *service) getSystemDiskFree(w http.ResponseWriter, r *http.Request) {
	...
	if resolved == "" {
		http.Error(w, "no usable path", http.StatusBadRequest)
		return
	}
	u, err := fs.NewFilesystem(fs.FilesystemTypeBasic, resolved).Usage(".")
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
// postFolderResync repairs a folder whose root or marker has gone
// missing. Re-creates the project folder on disk (if absent), writes
// the .vidsync marker, and kicks off a scan so the daemon transitions
// out of the ErrPathMissing / ErrMarkerMissing state and starts
// syncing again. Used by the "Resync" button the UI renders next to
// the Activity tab when one of those errors is current.
func (s *service) postFolderResync(w http.ResponseWriter, r *http.Request) {
	...
	if err := fcfg.CreateRoot(); err != nil {
		http.Error(w, "create root: "+err.Error(), http.StatusInternalServerError)
		return
	}
	if err := fcfg.CreateMarker(); err != nil {
		http.Error(w, "create marker: "+err.Error(), http.StatusInternalServerError)
		return
	}
	if err := s.model.ScanFolderSubdirs(folder, nil); err != nil {
