...
func (c *configMuxBuilder) adjustFolder(w http.ResponseWriter, r *http.Request, folder config.FolderConfiguration, defaults bool) {
	...
	waiter, err := c.cfg.Modify(func(cfg *config.Configuration) {
		...
		// Refuse to point a folder at a path that overlaps another
		// folder's tree — same path, a subfolder, or a parent. Checked
		// here (not in prepare()) so it only blocks new additions/edits,
		// never the loading of an already-overlapping config.
		if err := cfg.CheckFolderPathOverlap(folder); err != nil {
			errMsg = err.Error()
